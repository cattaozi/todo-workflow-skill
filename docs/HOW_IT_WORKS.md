...
**活跃项 = 扁平待办池**：不按 phase / 主题 / 月份分组，一堆松散 TODO 平铺即可。

**专项（Epic）= 活跃项之外唯一的分组**：把"同一件事拆出的多个 TODO"归到 `## 专项` 下 `### Epic：<名>` 表，结构和活跃表一致。成员状态流转和活跃项一样（done→归档、on_hold→搁置，留 `epic:` 标）。说"推进 Epic：<名>"，AI 按依赖序逐个推成员、每条门槛照常。
