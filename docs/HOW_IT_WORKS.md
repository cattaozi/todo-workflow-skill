...
**专项（Epic）**：TODO index 在 ## 活跃项 之上还有 `## 专项`——把"同一件事拆出的多个 TODO"归到 `### Epic：<名>` 下，表结构和活跃表一致。成员状态流转和活跃项完全一样（done→归档、on_hold→搁置，留 `epic:` 标）。说"推进 Epic：<名>"，AI 按依赖序逐个推成员、每条门槛照常。`epic:`（执行批次）与 `prd:`（出处）**正交可并存**——大 PRD 里若干 TODO 组成连贯子工程照样开 Epic，Epic 也能跨 PRD；唯一禁忌是别让 Epic 成为某 PRD 全部 TODO 的 1:1 复述。
