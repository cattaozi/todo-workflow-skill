...
**专项（Epic）**：TODO index 在 ## 活跃项 之上还有 `## 专项`——把"同一件事拆出的多个 TODO"归到 `### Epic：<名>` 下，表结构和活跃表一致。成员状态流转和活跃项完全一样（done→归档、on_hold→搁置，留 `epic:` 标）。说"推进 Epic：<名>"，AI 按依赖序逐个推成员、每条门槛照常。用于**没有 PRD 的工程倡议**（重构/迁移）；有 PRD 则 PRD 已分组，不另开 Epic。
