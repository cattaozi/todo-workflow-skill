...
**专项（Epic）= 活跃项之外唯一的分组**：把"同一件事拆出的多个 TODO"归到 `## 专项` 下 `### Epic：<名>` 表（列 = 活跃表 + `原因/备注`）。**成员永不迁出**——done/abandoned/on_hold 都在原行改状态 emoji、`原因/备注` 列记产出/理由/复活条件，所以 Epic 表是这件事的**完整台账**，回顾一眼看全。说"推进 Epic：<名>"，AI 按依赖序逐个推成员、每条门槛照常。
