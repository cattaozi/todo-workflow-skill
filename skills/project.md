...
`outbox/` 是 luca 的产出口袋，放我生成且需要保存的内容。
...
1. 当前 luca 壳仓。
2. `projects/` 已存在时检查项目域本地仓；存在但还不是本地仓时，按“本地仓启用”处理。
...
- 端口占用、后台托管、长会话退出、进程迁移或守护方式选择，先回到服务定义和启动文件判断，不直接套通用进程管理经验。
...
当用户语义是在移除某个已引入项目或外部资源时，含义是把它移出 luca 的项目域管理。核心动作是删除 `projects/links/<name>` 软链；默认只解除管理关系，不删除真实项目目录。
