...
- 你明确说“把项目域变成本地仓 / 初始化 projects git / 管一下 projects 的变化”。
- 第一次引入外部资源并创建项目域时。
...
5. 不自动提交；只有你要求“提交项目域 / commit projects”时，才在 `projects/` 内提交。
...
当你说“移除项目 / 移出项目 / 不再管理这个项目 / 从 luca 里去掉这个项目”时，默认含义是：把外部资源移出 luca 的项目域管理，不删除真实项目目录。
