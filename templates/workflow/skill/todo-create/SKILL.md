...
>
> **属于 Epic（专项）**：若这条 TODO 是某个"一件事拆出的多 TODO"的一员，frontmatter 标 `epic: "<名>"`，加到索引 `## 专项` 下对应 `### Epic：<名>` 表（**不进 ## 活跃项**）。详见 [`../../todo/README.md`](../../todo/README.md) §专项约定。
...
- `## 背景 / Why` —— **整条 TODO 的总纲**：为什么做 + 关键约束/取舍（不只写动机，把驱动实现的非显然约束也写进来，让人读懂下面任务为什么这么拆）
- `## 任务`（checkbox 列表）
- `## 完成定义（DoD）`
- `## 验收条件（AC）`（可暂留 TBD）
- `## 关联`
...
- `## 关键决策 / 方案` —— 只在有"非显然、未来会被问为什么这么建"的决策时写。装**长存的设计理由**（区别于做完即作废的任务）；事前想清的写这（=方案），事中/事后冒出的也补这（=关键决策）。没有就不写。
...
3. **创建详情文件** `TODO_NNN_<短描述>.md`：
   - 完整 frontmatter（含 `prd` / `epic` / `priority` 关联字段）
...
- ✅ 有明确的 DoD（AC 可暂留 TBD）
- ✅ 可独立 commit、独立 demo
- ❌ 不是"一周的工程"或"一个大阶段"（那种拆成多个 TODO，需要的话归一个 Epic）
- ❌ 不是"未来某个想法"（那是 EXP）
- ❌ 不是"5 分钟的改动"（直接做）
