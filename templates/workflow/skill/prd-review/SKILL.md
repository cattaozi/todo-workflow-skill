...
3. **与现状冲突**：跟 `DESIGN.md`（房间体系、门/院子/屋内隐私架构）、`CLAUDE.md`（默认 private 等铁律）、`docs/tech-stack.md`（不引入计划外依赖）有没有矛盾？
4. **维度混淆**：同一份数据是否在不同地方有不同结构、不同生命周期？
5. **自指依赖**：A 依赖 B，B 又依赖 A？
6. **隐私边界**：新内容类型默认是 `public` 还是 `private`？如果默认 public，直接拒绝。
...
- **隐私 / 默认 private 规则不可妥协**——任何 PRD 暗示"默认公开"都要在 PRD-R 里挑出来
