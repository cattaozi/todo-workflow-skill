...
> **行在哪张表就从哪张表迁**：Epic 成员的迁移规则和活跃项**完全一样**（done/abandoned → 归档、on_hold → 搁置），只是它起点在 `## 专项` 下的 `### Epic：<名>` 表。迁出后保留 frontmatter `epic:` 标，归属可追溯。
...
5. **每完成一个就更新 index**：done 成员从 Epic 表迁到归档（带 `epic:` 标）。
6. **全部 done/abandoned** → 删掉该 `### Epic：<名>` 子段，向用户报告 Epic 收尾。
...
- [ ] `index.md` 中该条目移到了正确分区（专项 / 活跃 / 搁置 / 归档）
