...
- 若执行中冒出**非显然决策**（换了选型 / 撤销了 PRD 写的方案 / 踩了非常规坑），补进 `## 关键决策 / 方案`（该段事前没写就新建，写过就追加）；常规执行不必赘述，commit message 已记够。这段是**长存的设计理由**，跟做完即作废的任务清单不是一回事
...
> **Epic 成员是例外：成员不拆散**。上面"迁到归档 / 搁置"只对**散户 TODO**（活跃池里、不属于任何 Epic 的）。**Epic 成员**任何状态都留在自己的 `### Epic：<名>` 表，**原行改 emoji + 填 `完成`（done/abandoned 填日期）+ `原因/备注` 列**（🟢 选填产出 / ⚫ 必填放弃理由 / ⏸️ 必填复活条件）——这样 Epic 表就是这件事的完整台账。
>
> **但整张 Epic 表会随完结而搬家**：当**最后一个成员**转 🟢/⚫（Epic 全完结），把**整个 Epic 块从 `## 专项` 迁到 `## 已完结专项`**（在 `## 归档` 之前，成员行不拆）。目的：`## 专项` 只留在做的 Epic，日常预览不被一堆绿表淹没。见 [`../../todo/README.md`](../../todo/README.md) §排序规则（已完结专项按 Epic 完成日期降序）。
...
5. **每完成一个就更新 index**：done 成员**在 Epic 表原行**改 🟢 + 填 `完成` 日期 + `原因/备注` 列填产出（**成员不拆**）。
6. **全部 🟢/⚫** → **把整个 Epic 块从 ## 专项 迁到 ## 已完结专项**（归档前，成员不拆），向用户报告 Epic 收尾。
...
- [ ] `index.md` 中该条目位置正确：散户 TODO 迁到对应分区（活跃 / 搁置 / 归档）；Epic 成员**原地**改状态 + 填 `完成`（done/abandoned）+ `原因/备注`
