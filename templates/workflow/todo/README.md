...
| **vs PRD（正交，不互斥）** | `prd:` 是**出处**（实现哪个需求），`epic:` 是**执行批次**（哪几条一起推）。一条 TODO 两者可并存。大 PRD 拆出几十条 TODO，其中若干条组成连贯子工程 → 照样开 Epic；Epic 也能跨多个 PRD。**唯一禁忌**：别让 Epic 等于"某 PRD 全部 TODO 的 1:1 复述"——那才冗余，直接用 PRD 的关联列表 |
...
- ❌ 给 < 3 个 / 松散相关的 TODO 硬开 Epic
- ❌ 让 Epic 成为某个 PRD 全部 TODO 的 1:1 复述（那用 PRD 关联列表就够）
